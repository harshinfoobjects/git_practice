...
object PowerBIStreaming2 {
    ...
    try {
     ...
     if (secure) {
      ...
      val hiveContext = getHiveContext(ssc.sparkContext, propertiesBroadcast)
       val dfPath=  propertiesBroadcast.value.getProperty("dataFramePath")
      ...
      // load latest records from raw layer elec_assy.ea_equipmentoutline table
          ...
          hiveContext.read.format("parquet").load(dfPath+"/equipmentoutline").registerTempTable("ea_equipment_outline_parquet")
        val equipDfParquet = hiveContext.sql("select tmp.machine_name, tmp.equipment_make, tmp.equipment_model,"
          ...
          + " head_2, plant, row_number() over (partition by machine_name , plant order by hdfs_file_name desc) row_num from " + "ea_equipment_outline_parquet" + " )"
          + " tmp where tmp.row_num = 1")
          
          


        broadcastedEquipmentOnline = ssc.sparkContext.broadcast(equipDfParquet.withColumn("updateTableTimestamp", lit(java.lang.System.currentTimeMillis())))
      ...
      /*  val infoDfHive = hiveContext.sql("select tmp.crc_id, tmp.name, tmp.description, tmp.plant_code, tmp.machine from (select crc_id, name,"
          ...
          * 
          */
        hiveContext.read.format("parquet").load(dfPath+"/ea_pap_info").registerTempTable("ea_pap_info_parquet")
        val infoDfParquet = hiveContext.sql("select tmp.crc_id, tmp.name, tmp.description, tmp.plant_code, tmp.machine from (select crc_id, name,"
          + " description, plant_code, machine, row_number() over (partition by plant_code, machine, crc_id order by hdfs_file_name desc) row_num from " + "ea_pap_info_parquet" + " )"
          + " tmp where tmp.row_num = 1")

        

        broadcastedInfo = ssc.sparkContext.broadcast(infoDfParquet.withColumn("updateTableTimestamp", lit(java.lang.System.currentTimeMillis())))
        ...
        hiveContext.read.format("parquet").load(dfPath+"/wodetail").registerTempTable("ea_wo_detail")
        val wodetailDfParquet = hiveContext.sql("select tmp.optel_schedule_wo, tmp.sap_wo, tmp.side, tmp.assembly, tmp.plant, tmp.setup, tmp.wo_detail from (select optel_schedule_wo,sap_wo,"
        ...
        + " row_number() over (partition by plant , optel_schedule_wo  order by hdfs_file_name) row_num  from " + "ea_wo_detail" + ")" 
        + " tmp where tmp.row_num = 1")
          
        

      broadcastedWoDetail = ssc.sparkContext.broadcast(wodetailDfParquet.withColumn("updateTableTimestamp", lit(java.lang.System.currentTimeMillis())))
      ...
      // load latest records from raw layer elec_assy.ea_progorderlist table
        ...
        hiveContext.read.format("parquet").load(dfPath+"/progorderlist").registerTempTable("ea_progorderList")
        val progorderListDfParquet = hiveContext.sql("select tmp.machine, tmp.plant, tmp.work_order, tmp.ref_id, tmp.silkscreen, tmp.circuit, tmp.head, tmp.spindle, tmp.head_spindle, tmp.ref_des,"
          ...
          + " substr(ref_id,length(circuit)+2) as ref_des, row_number() over (partition by machine,plant,work_order,substr(ref_id,length(circuit)+2),silkscreen order by hdfs_file_name desc) row_num  from " + "ea_progorderList" + " )"
          + " tmp where tmp.row_num = 1")

          

        broadcastedProgorderList = ssc.sparkContext.broadcast(progorderListDfParquet.withColumn("updateTableTimestamp", lit(java.lang.System.currentTimeMillis())))
      ...
      // load latest records from raw layer elec_assy.ea_comptype table
          ...
          hiveContext.read.format("parquet").load(dfPath+"/ea_comptype").registerTempTable("ea_comptype")
        val compTypeDfParquet = hiveContext.sql("select tmp.plant, tmp.machine, tmp.part_number, tmp.head_type, tmp.pins, tmp.nozzle1, tmp.package from ("
          + " select plant, machine, part_number, head_type, pins, nozzle1, package, row_number() over (partition by plant , machine , part_number, head_type order by hdfs_file_name) row_num from " + "ea_comptype" + " )"
          + " tmp where tmp.row_num = 1")

        broadcastedCompType = ssc.sparkContext.broadcast(compTypeDfParquet.withColumn("updateTableTimestamp", lit(java.lang.System.currentTimeMillis())))
      ...
      mappedDataStreamC4.foreachRDD { rdd =>
        try {
          ...
          if (!rdd.isEmpty()) {
            ...
            // load all the data frames from a kafka topic 
            val nozzleDf = hiveContext.read.json(rdd.filter(ele => ele.contains(propertiesBroadcast.value.getProperty("ea_pap_nozzle_header"))))
            val timersDf = hiveContext.read.json(rdd.filter(ele => ele.contains(propertiesBroadcast.value.getProperty("ea_pap_timers_header"))))
            val slotDf = hiveContext.read.json(rdd.filter(ele => ele.contains(propertiesBroadcast.value.getProperty("ea_pap_slot_header"))))
            val defectDf = hiveContext.read.json(rdd.filter(ele => ele.contains(propertiesBroadcast.value.getProperty("ea_ftdefects_header"))))
            ...
            if (!nozzleDf.rdd.isEmpty()) {
              ...
              if (!timersDf.rdd.isEmpty()) {

              println("new timer data");

              timersDf.withColumnRenamed("ulProductionTime", "ul_production_time")
             ...
             if (!slotDf.rdd.isEmpty()) {
              ...
              query.append("A.ul_possible_missing, A.ul_slot_picks, A.ul_slot_placements, A.ul_slot_vision_failures,")
  ...
  def refreshLocPlantXref(sc: SparkContext, propertiesBroadcast: Broadcast[Properties]): Unit = {
     ...
     val locPlantXrefDf = sc.parallelize(Seq(("twb",1020),("meq",1020),("sng",5170),("mmc",1150))).toDF("location","plant")
     locPlantXrefDf.show()
     locPlantXrefDf.registerTempTable("elctrnc_assy_loc_plant_xref")
