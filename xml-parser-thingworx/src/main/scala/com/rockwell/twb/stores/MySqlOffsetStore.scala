package com.rockwell.twb.stores
...
class MySqlOffsetsStore(jdbcUsername: String, jdbcPassword: String, jdbcHostname: String, jdbcPort: Int, jdbcDatabase: String) extends OffsetsStore {
   ...
   */
  override def readOffsets(topic: String,topic_tail_constant: String): Option[Map[TopicAndPartition, Long]] = {

    val resultSet = statement.executeQuery("select * from topic_partitions_offset where topic = '" + (topic+topic_tail_constant) + "'")
   ...
   *  workaround.
   */
  override def saveOffsets(dataStream: InputDStream[(String, String)],topic_tail_constant: String): Unit = {
    ...
    dataStream.transform { rdd =>
      ...
      for (i <- 0 to offsetRanges.length - 1) {
        val topic = offsetRanges(i).topic + topic_tail_constant
        ...
        if (resultSet.next()) {
          //println("Record found in mysql hence updating.........")
  ...
  def deleteOffsets(topic: String):Unit = {
      println("deleting offsets to Mysql")
