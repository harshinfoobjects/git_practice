...
object HiveService {
  ...
  def processStream(dataStream: InputDStream[(String, String)], propertiesBroadcast: Broadcast[Properties],topic:String,offsetStore: MySqlOffsetsStore) {
    val mappedDataStream = dataStream.map(_._2);
    // dataStream.repartition(3*nodecount)??
    mappedDataStream.foreachRDD { rdd =>
      try {
      var headerNotFound = true
      if (!rdd.isEmpty()) {
        val sparkContext = rdd.sparkContext
        if ("cgp_kafka_spi_header_ea_ra_out".equalsIgnoreCase(propertiesBroadcast.value.getProperty("inletKafkaTopic"))) {
          headerNotFound = false
          insertInto_ea_spi_header(getInstance(sparkContext, propertiesBroadcast).read.json(rdd), propertiesBroadcast)

        } else if ("cgp_kafka_spi_detail_ea_ra_out".equalsIgnoreCase(propertiesBroadcast.value.getProperty("inletKafkaTopic"))) {
          headerNotFound = false
          insertInto_ea_spi_detail(getInstance(sparkContext, propertiesBroadcast).read.json(rdd), propertiesBroadcast)

        } else if ("cgp_kafka_pap_ea_ra_out".equalsIgnoreCase(propertiesBroadcast.value.getProperty("inletKafkaTopic"))) {

          val rdd_timers = rdd.filter {
            ele => ele.contains(propertiesBroadcast.value.getProperty("ea_pap_timers_header"))
          }
          if (!rdd_timers.isEmpty()) {
            headerNotFound = false
            insertInto_ea_pap_timers(getInstance(sparkContext, propertiesBroadcast).read.json(rdd_timers), propertiesBroadcast)
          }

          val rdd_slot = rdd.filter {
            ele => ele.contains(propertiesBroadcast.value.getProperty("ea_pap_slot_header"))
          }
          if (!rdd_slot.isEmpty()) {
            headerNotFound = false
            insertInto_ea_pap_slot(getInstance(sparkContext, propertiesBroadcast).read.json(rdd_slot), propertiesBroadcast)
          }

          val rdd_info = rdd.filter {
            ele => ele.contains(propertiesBroadcast.value.getProperty("ea_pap_info_header"))
          }
          if (!rdd_info.isEmpty()) {
            headerNotFound = false
            insertInto_ea_pap_info(getInstance(sparkContext, propertiesBroadcast).read.json(rdd_info), propertiesBroadcast)
          }

          val rdd_gen = rdd.filter {
            ele => ele.contains(propertiesBroadcast.value.getProperty("ea_pap_gen_header"))
          }
          if (!rdd_gen.isEmpty()) {
            headerNotFound = false
            insertInto_ea_pap_gen(getInstance(sparkContext, propertiesBroadcast).read.json(rdd_gen), propertiesBroadcast)
          }

          val rdd_nozzle = rdd.filter {
            ele => ele.contains(propertiesBroadcast.value.getProperty("ea_pap_nozzle_header"))
          }
          if (!rdd_nozzle.isEmpty()) {
            headerNotFound = false
            insertInto_ea_pap_nozzle(getInstance(sparkContext, propertiesBroadcast).read.json(rdd_nozzle), propertiesBroadcast)
          }

        } else if (propertiesBroadcast.value.getProperty("inletKafkaTopic").contains("cgp_kafka_masterdata_xyz_ra")) {
          val rdd_progorderlist = rdd.filter {
            ele => ele.contains(propertiesBroadcast.value.getProperty("ea_progorderlist_header"))
          }
          if (!rdd_progorderlist.isEmpty()) {
            headerNotFound = false
            insertInto_progorderlist(getInstance(sparkContext, propertiesBroadcast).read.json(rdd_progorderlist), propertiesBroadcast)
          }

          val rddftdefects = rdd.filter {
            ele => ele.contains(propertiesBroadcast.value.getProperty("ea_ftdefects_header"))
          }
          if (!rddftdefects.isEmpty()) {
            headerNotFound = false
            insertInto_ftdefects(getInstance(sparkContext, propertiesBroadcast).read.json(rddftdefects), propertiesBroadcast)
          }

          val rdd_assydetail = rdd.filter {
            ele => ele.contains(propertiesBroadcast.value.getProperty("ea_assydetail_header"))
          }
          if (!rdd_assydetail.isEmpty()) {
            headerNotFound = false
            insertInto_assydetail(getInstance(sparkContext, propertiesBroadcast).read.json(rdd_assydetail), propertiesBroadcast)
          }

          val rdd_units = rdd.filter {
            ele => ele.contains(propertiesBroadcast.value.getProperty("ea_units_header"))
          }
          if (!rdd_units.isEmpty()) {
            headerNotFound = false
            insertInto_units(getInstance(sparkContext, propertiesBroadcast).read.json(rdd_units), propertiesBroadcast)
          }

          val rdd_wodetail = rdd.filter {
            ele => ele.contains(propertiesBroadcast.value.getProperty("ea_wodetail_header"))
          }
          if (!rdd_wodetail.isEmpty()) {
            headerNotFound = false
            insertInto_wodetail(getInstance(sparkContext, propertiesBroadcast).read.json(rdd_wodetail), propertiesBroadcast)
          }

          val rdd_assycircuitsilk = rdd.filter {
            ele => ele.contains(propertiesBroadcast.value.getProperty("ea_assycircuitsilk_header"))
          }
          if (!rdd_assycircuitsilk.isEmpty()) {
            headerNotFound = false
            insertInto_assycircuitsilk(getInstance(sparkContext, propertiesBroadcast).read.json(rdd_assycircuitsilk), propertiesBroadcast)
          }

          val rdd_workorder = rdd.filter {
            ele => ele.contains(propertiesBroadcast.value.getProperty("ea_workorder_header"))
          }
          if (!rdd_workorder.isEmpty()) {
            headerNotFound = false
            insertInto_workorder(getInstance(sparkContext, propertiesBroadcast).read.json(rdd_workorder), propertiesBroadcast)
          }

          val rdd_parentchild = rdd.filter {
            ele => ele.contains(propertiesBroadcast.value.getProperty("ea_parentchild_header"))
          }
          if (!rdd_parentchild.isEmpty()) {
            headerNotFound = false
            insertInto_parentchild(getInstance(sparkContext, propertiesBroadcast).read.json(rdd_parentchild), propertiesBroadcast)
          }

          val rdd_comptype = rdd.filter {
            ele => ele.contains(propertiesBroadcast.value.getProperty("ea_comptype_header"))
          }
          if (!rdd_comptype.isEmpty()) {
            headerNotFound = false
            insertInto_comptype(getInstance(sparkContext, propertiesBroadcast).read.json(rdd_comptype), propertiesBroadcast)
          }

          val rdd_equipmentoutline = rdd.filter {
            ele => ele.contains(propertiesBroadcast.value.getProperty("ea_equipmentoutline_header"))
          }
          if (!rdd_equipmentoutline.isEmpty()) {
            headerNotFound = false
            insertInto_equipmentoutline(getInstance(sparkContext, propertiesBroadcast).read.json(rdd_equipmentoutline), propertiesBroadcast)
          }
        }
        if (headerNotFound) {
          println("*************************************************")
  ...
  def insertInto_hive(readyToSaveDf: DataFrame, tableName: String, propertiesBroadcast: Broadcast[Properties]) {
    ...
    UserGroupInformation.loginUserFromKeytabAndReturnUGI(
      ...
      .doAs(new PrivilegedExceptionAction[Unit]() {
        ...
        def run(): Unit = {
          ...
          refreshTable(tableName, propertiesBroadcast)
          //readyToSaveDf.write.mode(SaveMode.Append).format("parquet").saveAsTable(tableName)
  ...
  def refreshTable(tableName: String, propertiesBroadcast: Broadcast[Properties]): Unit = {
    try {
  ...
  def main(args: Array[String]): Unit = {
    ...
    processStream(dataStream, propertiesBroadcast,topic+"_hive",offsetStore)
